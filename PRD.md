...
| 项目 | 内容 |
| --- | --- |
| 文档状态 | v0.2.0 已发布（仅一期，二期已废弃，见变更记录） |
...
| 二期价值 | ~~在该视图下额外给出一段"这段命令是干什么的"执行内容说明~~（**已废弃**，v0.2.0 移除，见变更记录） |
| 覆盖工具 | `bash` / `pwsh` 等"bash 系"命令行工具（参数名 `command`） |

> **变更记录（v0.2.0）**：本 PRD 最初规划两期——一期「指令格式化展示」、二期「指令执行内容说明（由模型生成）」。实际交付时，二期（AI 解释）因第三方持久包做 client→host 调用在官方 client remote 静态装配下不可靠、且一度影响审批可用性，已**整体废弃移除**。当前产品范围**仅保留一期**：命令语法高亮格式化，纯客户端、确定性、无网络，且不改变审批语义。下文 §6.2（执行内容说明）、§8.3（模型解释）、§8.5（Host↔Client 契约）、M2 验收等二期内容仅供历史参考，不再实现。
