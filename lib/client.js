...
function makeApprovalView(getExplainer) {
...
function apply(ctx) {
	...
	if (remote && typeof remote.$mount === "function") ctx.effect(() => {
		let cancelled = false;
		let disposeRemote;
		...
		return () => {
			cancelled = true;
			if (disposeRemote) disposeRemote();
		};
	...
	const getExplainer = () => ctx.get("remote.commandExplainer");
